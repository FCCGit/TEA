...
class TEAC
{
	...
	function get_xml($type, $post = NULL)
	{
		...
		$cache = FALSE;
		if($type != 'standings' && $type != 'alliances' && $type != 'api' && method_exists($this, 'get_cache'))
		{
		...
		if($type != 'standings' && $type != 'alliances' && $type != 'api' && method_exists($this, 'set_cache'))
		{
	...
	function get_api_details($userid, $api)
	{
		$post = array('keyID' => $userid, 'vCode' => $api);
		$api = $this -> get_xml('api', $post);
		$this -> data = $api;
		$eveapi = simplexml_load_string($api);

		$key = $eveapi->result->key;
		if(is_null($key)){
			return NULL;
		} else {
			return array("mask" => $key->attributes()->accessMask, "type" => $key->attributes()->type, "expire" => $key->attributes()->expires); 
		}
	}
